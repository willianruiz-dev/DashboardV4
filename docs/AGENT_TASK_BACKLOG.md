...
| 6 | Control de dispensado | **IMPLEMENTADO (F1–F4); PENDIENTE DATOS + E2E** | `src/features/dispensing-control/` + ruta `/dashboard/transactions/dispensing-control`. useDispensingMetrics orquesta storage/arqueos/cargues/búsqueda (summary sobre el conjunto completo vía byState, extensión BFF aplicada). Guard data-driven por ruta asignada al rol SuperAdmin (D1); umbral minDpQuantity+10 (D2). Pendiente: crear la ruta `/Admin/Transactions/DispensingControl` en el módulo Rutas, asignarla al rol SuperAdmin en Roles, y E2E autenticado con Pay+ real. Ver `docs/DISPENSING_CONTROL_FEASIBILITY.md`. |
