...
| --- | --- | --- | --- |
| 0 | Sistema de diseño | **PASA (código + build); PENDIENTE E2E visual** | Tailwind v4 CSS-first, tokens del manual, tema claro inicial/oscuro seleccionable y primitives. Las superficies de estado son pastel semánticas, botones tienen elevación suave y Pay+ usa cards de identidad de máquina. El banner E-city existente aparece en login, cabecera y navegación. |
...
- Sistema visual 2026: `globals.css` mantiene exclusivamente los tokens de `ManualDeMarca.md` y deriva superficies pastel semánticas con contraste AA. Transacciones usa verde para aprobadas, rojo para canceladas y azul para totales; Consultar/Excel usan variante success y Ver detalle una acción gris. Los cards Pay+ ganan elevación, rail de estado, icono de máquina y controles consistentes sin cambiar datos, permisos ni acciones.
...
| Criterio | Resultado |
| --- | --- |
| Tipado estricto / sin `any` | **PASA** — `npm run typecheck` (2026-09-17) |
| ESLint sin warnings | **PASA** — `npm run lint` (2026-09-17) |
| Build de producción | **PASA** — `npm run build` (2026-09-17) |
...
| Sistema visual Transacciones / Pay+ | **PASA (código + build)** — tokens semánticos pastel, transiciones de 300 ms, elevación suave, hover de filas y cards Pay+ con indicador de estado/iconografía. La inspección en sesión real sigue pendiente. |
