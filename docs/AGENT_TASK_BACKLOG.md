...
| 3 | Cargues | **IMPLEMENTADO; PENDIENTE E2E** | `LoadDto.details` y la respuesta nullable documentada se normalizan a listas; el cargue conserva denominación, cantidad y total, usa el nombre de máquina y solicita el billete histórico automáticamente. Falta probar con un Pay+ productivo. |
| 4 | Arqueos | **IMPLEMENTADO; PENDIENTE E2E** | La respuesta nullable de arqueos se normaliza a lista; el payload vuelve a enviar `total`, `totalAp`, `totalDp` y `totalRj`, el total general se toma de storage y el historial presenta los resúmenes del legado. Falta prueba con un Pay+ productivo. |
