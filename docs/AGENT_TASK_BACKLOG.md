...
| 3 | Cargues | **ADAPTADOR DE LECTURA LEGADO APLICADO; PENDIENTE E2E** | La consulta usa la misma ruta BFF/upstream del legado y valida sólo los campos que `PayPadBalanceView` realmente renderiza: ID, fecha, total, denominación y cantidad. Los metadatos de envelope y relaciones no usadas ya no bloquean el listado; los IDs de payload de cargue siguen siendo estrictamente positivos. Falta contrastarlo con el payload autenticado de un Pay+ productivo. |
| 4 | Arqueos | **ADAPTADOR DE LECTURA LEGADO APLICADO; PENDIENTE E2E** | Se conserva la misma adaptación limitada para ID, fecha, totales y detalles de arqueo; el panel muestra sus estados y reintento de forma independiente al de cargues, como las consultas separadas del legado. El payload conserva `total`, `totalAp`, `totalDp` y `totalRj`. Falta contrastarlo con datos productivos. |
...
- El historial de cargues y arqueos usa las mismas rutas `Load/GetByPaypad` y `Tonnage/GetByPaypad` que el legado, pero adapta tipadamente sólo la forma que éste consume: `response` iterable y los campos visibles de cada movimiento. Relaciones no visibles (`idPayPad`, `idLoad`, `idTonnage`) y metadatos de envelope no impiden el render; un `response` que no sea lista o un importe/cantidad/ID visible inválido continúa fallando. La imagen de denominación usa su ID válido y, sólo si falta, el valor de la denominación. Esta compatibilidad no se aplica a los schemas ni endpoints de escritura financiera.
...
| Adaptador de historial legado | **PASA (integración local)** — los dos clientes solicitan las rutas relativas originales y aceptan un envelope con sólo `response`, IDs relacionales ausentes y detalles sin relación de denominación; una respuesta sin lista o campos financieros/visibles inválidos sigue rechazándose. Las mutaciones continúan exigiendo IDs positivos. |
...
6. **B-06 — Forma runtime de historial:** la captura productiva demuestra que el parser anterior rechazaba un 2xx de cargues/arqueos. El código fuente y Swagger no incluyen un ejemplo autenticado de ese movimiento. Se reemplazó la validación de campos que el dashboard antiguo nunca leía por un adaptador tipado de los campos que sí presenta, manteniendo la misma ruta, HTTP real, requerimiento de `response` iterable y validación de ID/importe/cantidad visibles. No se convierte un registro inválido en una lista vacía. La respuesta autenticada real sigue pendiente para confirmar la paridad.

## ESTADO DEL BACKLOG

No se declara la migración terminada. Se retiró el supuesto erróneo de que el API productivo hospedaba `/staticfiles` o `/images`: la ruta de imágenes usa ahora el host legado verificable `dashboardv2.e-city.co/staticfiles`, devuelve archivos reales de los paths DB y conserva el acceso same-origin desde la aplicación. En este ciclo el historial se alineó con la lectura real de `PayPadBalanceView`, cada panel quedó aislado ante el fallo del otro y Transacciones/Reportes incorporaron rango diario legado, filtro Efectivo/Tarjeta y orden server-side determinista. Restan pruebas E2E autenticadas de la interfaz —incluidos billetes/denominaciones, cargues, arqueos, Excel, vídeo y controles visibles— antes de declarar paridad funcional completa.
