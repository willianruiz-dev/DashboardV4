...
- Los paths históricos de imagen (`/images/...`, `/staticfiles/images/...`, barras inversas y URLs absolutas) se normalizan a un path relativo seguro `/staticfiles/...`; cadenas `NULL`/`undefined`, traversal y separadores codificados repetidamente se rechazan. El BFF adjunta `DashboardKeyId` y, cuando existe, el Bearer de la sesión HttpOnly sólo server-side; sigue redirects y, si el virtual directory `/staticfiles/images/...` falla o responde un documento de error, intenta la ruta DB directa `/images/...` sin exponer URLs upstream al navegador.
...
| Contrato de estáticos | **PASA (integración local)** — mock HTTPS verificó `DashboardKeyId`, Bearer sólo con sesión, redirects y el fallback automático de `/staticfiles/images/...` a `/images/...`; el navegador recibe sólo el binario same-origin. |
