...
- La detección de atascos analiza **automáticamente** al seleccionar una máquina o cambiar el período (sin botón previo): la consulta se cachea por máquina+rango (`staleTime` 5 min) y el BFF mantiene 30 min de caché por `id` de transacción, con tope de 40 transacciones por análisis y concurrencia 5.
- La atribución distingue **culpable** de **compensador**: una denominación que entrega más de su parte canónica no recibe señales de atasco (está cubriendo a otra). La existencia del monedero se deduce del saldo en el baúl dispensador (actual o histórico), no de `isDispensing`, porque un módulo atascado no se mueve y su configuración puede estar vencida.
