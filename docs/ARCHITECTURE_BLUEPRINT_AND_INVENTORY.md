...
- De acuerdo con el Swagger productivo, `LoadDto.details`, `TonnageDto.details` y los envelopes de lista nullable de cargues, arqueos y almacenamiento se normalizan de `null` o ausencia a `[]`; así un Pay+ sin movimientos o inventario muestra el estado vacío en vez de un error de formato. Para los dos historiales, el adaptador de **sólo lectura** replica el contrato efectivo de `PayPadBalanceView`: lee `response` y los campos que esa vista presenta (ID, fecha, totales, denominación y cantidades), no bloquea por metadata de envelope ni relaciones que la vista antigua no consumía, y continúa rechazando un `response` no iterable o valores visibles inválidos. Las mutaciones financieras conservan `.positive()`.
...
| Adaptador de historial legado | **PASA (integración local)** — los clientes usan `/api/backend/api/Load/GetByPaypad/:id` y `/api/backend/api/Tonnage/GetByPaypad/:id`, aceptan la forma de lectura del dashboard antiguo (sin metadata ni relaciones no visuales), y preservan el rechazo de lista/valores visibles inválidos. Las mutaciones siguen rechazando IDs `0`. |
...
| B-06 | La captura de producción demuestra que el parser anterior rechazaba un 2xx de cargues/arqueos, pero Swagger y el código no incluyen una muestra autenticada de ese registro. | No se puede documentar cada campo runtime no visual. | El adaptador ahora sigue el consumo del componente legado: valida sólo `response` iterable y los campos que éste renderiza. Sigue propagando HTTP, rechaza valores visibles inválidos y no transforma registros defectuosos en listas vacías. La respuesta autenticada real sigue pendiente. |

## ESTADO DEL BACKLOG

La arquitectura no se presenta como cierre de producción. Se reemplazó la suposición incorrecta de que el API hospedaba los archivos por el origen estático verificable que el frontend anterior usaba: `dashboardv2.e-city.co/staticfiles`. El BFF mantiene las URLs relativas y secretos en servidor, pero ahora recupera los bytes reales del directorio histórico en vez de un fallback inventado. Este ciclo alineó Cargues/Arqueos con el lector efectivo del dashboard antiguo, aisló sus errores por panel y añadió al flujo de Transacciones/Reportes el rango diario legado, filtro Efectivo/Tarjeta y orden server-side comprobable. Aún es obligatoria la comprobación autenticada de las pantallas con datos reales —billetes/denominaciones, cargues, arqueos, Excel, vídeo y controles— antes de declarar paridad funcional completa.
