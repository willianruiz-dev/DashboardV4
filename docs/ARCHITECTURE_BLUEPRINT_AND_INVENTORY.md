...
| API BFF | El navegador consume rutas relativas. `src/app/api/backend/[...path]/route.ts` agrega `DashboardKeyId` y token sólo al API productivo en el servidor. |
...
| `/dashboard/paypads` | `/Admin/PayPad` | CRUD, contraseña, almacenamiento y flujos de Pay+ implementados; Cargues/Arqueos siguen fallando en la última prueba autenticada de Prueba1 y Configuración propaga un fallo SQL upstream. |
...
- De acuerdo con Swagger, `LoadDto.details`, `TonnageDto.details` y los envelopes de lista nullable se manejan sin tratar la ausencia de movimientos como error. Sin embargo, la última prueba autenticada de Pay+ Prueba1 sigue rechazando ambos historiales con un error de formato: el contrato runtime aún no está demostrado. El adaptador de sólo lectura no se ampliará por hipótesis ni transformará registros fallidos en `[]`. Durante `npm run dev`, el BFF registra una huella segura del GET exacto —nombres de campo, tipos JSON, tamaños y paths Zod, nunca valores/cookies/tokens— y el error visible incluye hasta tres paths sanitizados para obtener evidencia de la discrepancia.
...
| Adaptador de historial legado | **PASA sólo en integración local; FALLA E2E autenticado** — las rutas relativas y el mock pasan, pero ambas consultas de Prueba1 siguen rechazando el 2xx real. No se declara compatibilidad hasta obtener la estructura autenticada. |
...
| Cargues y arqueos con sesión real (Pay+ Prueba1) | **FALLA E2E** — ambos muestran “La aplicación recibió una respuesta con formato inesperado”; la corrección queda bloqueada hasta capturar el diagnóstico estructural. |
...
| B-06 | La captura autenticada posterior demuestra que Cargues y Arqueos de Prueba1 continúan rechazando el 2xx real, aunque el mock y el adaptador inicial pasen. Swagger y el código no contienen ese cuerpo. | No se puede cambiar otro campo sin inventar una variante de contrato. | El BFF registra únicamente estructura/tipos/paths Zod sanitizados al usar `npm run dev`; el mensaje de ambos paneles incluye esos paths sin valores. Se debe corregir sólo la discrepancia que revele esa evidencia. |
...
La arquitectura no se presenta como cierre de producción. Aunque el BFF conserva URLs relativas, secretos server-side y el origen histórico real de estáticos, las capturas autenticadas invalidan la afirmación previa de compatibilidad de historial: Cargues y Arqueos de Prueba1 siguen fallando. Este ciclo elimina un defecto objetivo al relay de respuestas comprimidas y agrega una traza estructural que no expone valores ni credenciales para obtener el contrato runtime antes de tocar Zod. El error SQL de `VALIDATE_PERIPHERALS` permanece separado y visible como problema backend. Sigue siendo obligatoria la corrección basada en esa evidencia y la comprobación autenticada de las pantallas con datos reales —billetes/denominaciones, cargues, arqueos, Excel, vídeo y controles— antes de declarar paridad funcional completa.
