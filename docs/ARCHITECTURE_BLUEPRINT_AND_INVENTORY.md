...
La arquitectura no se presenta como cierre de producción. La captura autenticada permitió sustituir una hipótesis por un contrato concreto: los movimientos históricos de Prueba1 pueden tener cantidades negativas, que ahora se preservan sólo al leer. También se recuperó una presentación funcional de tabla, responsable, detalle y filtro para escritorio, con cards para móvil. Falta la comprobación autenticada posterior que confirme ambos historiales y detalles de Prueba1. El error SQL de `VALIDATE_PERIPHERALS` permanece separado y visible como problema backend. Sigue siendo obligatoria esa prueba, junto con billetes/denominaciones, Excel, vídeo y controles, antes de declarar paridad funcional completa.
