...
- De acuerdo con el Swagger productivo, `LoadDto.details`, `TonnageDto.details` y los envelopes de lista nullable de cargues, arqueos y almacenamiento se normalizan de `null` o ausencia a `[]`; así un Pay+ sin movimientos o inventario muestra el estado vacío en vez de un error de formato. Para la captura runtime que rechazaba ambos historiales, los schemas de **sólo lectura** también aceptan el centinela numérico `0` en `idPayPad` e `idCurrencyDenomination`, compatible con DTOs .NET `int` sin nulo. Las mutaciones financieras conservan `.positive()` y las demás desviaciones de formato continúan fallando en Zod.
...
| Contrato Zod de cargues/arqueos | **PASA (contrato local)** — `details: null`/ausente se vuelve `[]`; envelopes con los centinelas de lectura `idPayPad: 0` e `idCurrencyDenomination: 0` pasan por `httpEnvelopeSchema`, mientras un objeto inválido y los IDs `0` de mutaciones siguen rechazándose. |
...
| B-06 | La captura de producción demuestra que el parser anterior rechazaba un 2xx de cargues/arqueos, pero Swagger y el código no incluyen una muestra autenticada de ese registro. | No se puede asegurar que el centinela `0` sea la única diferencia de serialización runtime. | Se incorporó un adaptador tipado, limitado a relaciones de lectura no usadas al mutar; conserva errores HTTP/envelope y no transforma registros inválidos en listas vacías. La respuesta autenticada real sigue pendiente. |

## ESTADO DEL BACKLOG

La arquitectura no se presenta como cierre de producción. Se reemplazó la suposición incorrecta de que el API hospedaba los archivos por el origen estático verificable que el frontend anterior usaba: `dashboardv2.e-city.co/staticfiles`. El BFF mantiene las URLs relativas y secretos en servidor, pero ahora recupera los bytes reales del directorio histórico en vez de un fallback inventado. Este ciclo repuso el banner E-city comprobado localmente, vinculó las transacciones a `PayPad.username` y acotó la compatibilidad de historial al centinela compatible. Aún es obligatoria la comprobación autenticada de las pantallas con datos reales —billetes/denominaciones, cargues, arqueos, Excel, vídeo y controles— antes de declarar paridad funcional completa.
